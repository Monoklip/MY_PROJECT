...
import './admin-account-moto-delete-item.scss';

const AdminAccountMotoDeleteItem = (props: {
    elem?: any;
    nameAutoDel?: any;
    modelAutoDel?: any;
}) => {
    ...
    const [delUrl, setDelUrl] = useState(`http://localhost:3000/moto/${id}`);
        ...
        if (model === modelAutoDel && modelAutoDel !== "all") {
            return (
                <div onMouseMove={move} onMouseLeave={leave} className="list-carDel">
                    {icon &&
                        <button onClick={deleteMoto} className="list-carDel-deleteIcon">
...
export default AdminAccountMotoDeleteItem;

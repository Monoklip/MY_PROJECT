import './admin-account-post.scss';
...
const AdminAccountPost = () => {
    ...
    async function postCar() {
        try {
            if(postCarName !== 'none' && postCarModal !== 'all' && postCarYear !== 0 && postCarKm > 0 && postCarPrice > 0){
                await fetch(url, {
                    method: "POST",
                    body: JSON.stringify({
                        name: postCarName,
    ...
    return (
        <div className="admin-account-postCar">
            ...
            <button className="admin-account-postCar-btn" onClick={postCar}>
                Добавити авто
            </button>
...
export default AdminAccountPost;
